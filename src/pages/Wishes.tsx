      ...
      <div className="relative">
        <div className="h-24 w-full bg-yellow-400">
          <div className="h-full w-full bg-[url(/images/patterns/pattern.png)]">
            <Breadcrumb pageLink="علاقه مندی ها" />
          </div>
        </div>
        {wishes.length > 0 ? (
          <div className="container mt-5 grid grid-cols-1 gap-5 pb-20 sm:grid-cols-2 lg:grid-cols-3">
            {wishesRooms.map((room) => (
              <RoomsContainer isHost={true} room={room} key={room.id} />
            ))}
          </div>
        ) : (
          <div className="-mt-5 h-screen rounded-t-2xl bg-white dark:bg-gray-800">
            <div className="container flex flex-col items-center justify-center gap-5 py-16">
              <svg
                fill="none"
                viewBox="0 0 169 140"
              ...
              <div className="flex flex-wrap items-center justify-center gap-1 text-sm text-gray-500 dark:text-gray-300">
